...
parms <- get_salmo_parms(macrophytes=TRUE)


## Matrices are faster than data frames
forc <- as.matrix(forc)
vsedi <- matrix(c(0,0,parms$pp["VS",],0,0,0,0,0,0),nrow=1)
...
parms$pp_ma["cMaxHeightVeg", 1] <- 4.5 # maximum height of macrophytes [m]
parms$pp_ma["fDepth1Veg",1] <- 5/50    # upper fraction of the box up to which submerged macrophytes grow [-]
parms$pp_ma["fDepth2Veg",1] <- 50/50  # lower fraction of the box down to which submerged macrophytes grow [-]
...
### !!! simulation still crashes; macrophytes originally not intended for 1 Box.
